...
export class UserService {
	...
	async findById (id: string){
		...
		if (user) {
			...
			throw new HttpException("UserId provided is invalid!", HttpStatus.NOT_FOUND,);
	}
	...
	async findByUsername (username: string){
		...
		if (user)
			return user;
			throw new HttpException("Username provided is invalid!", HttpStatus.NOT_FOUND,);
	}
	...
	async updateDisplayName(id: string, registerData: ChangeDisplayNameDto) {
		try {
			...
			throw new HttpException('Error while update user displayname!', HttpStatus.INTERNAL_SERVER_ERROR,);
		}
	}
	...
	async updateImage(id: string, path: string) {
		try {
			...
			throw new HttpException('Error while update user image!', HttpStatus.INTERNAL_SERVER_ERROR,);
		}
		return undefined;
	}
	...
	async getImageId(id: string) {
		try {
			...
			throw new HttpException('Error while get user image!', HttpStatus.INTERNAL_SERVER_ERROR,);
		}
		return undefined;
	}
	...
	async getUserPublicData (userID: string) {
		...
		if (user)
			return user;
		throw new HttpException("Error in get user public information!", HttpStatus.NOT_FOUND,);
	}
	...
	async setTwoFactorAuthenticationSecret(secret: string, userId: string) {
		try {
			...
			throw new HttpException('Error in 2fa authentication!', HttpStatus.INTERNAL_SERVER_ERROR,);
		}
		return undefined;
	}
	...
	async updateWebSocketId(userId: string, socketId: string) {
		const user = await this.findById(userId);
		if (!user) {
			throw new HttpException('Error during socket update request!', HttpStatus.INTERNAL_SERVER_ERROR,);
		}
